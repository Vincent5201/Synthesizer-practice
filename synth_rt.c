...
q31_t note_to_phase(int note) {
    return octave_phases[note % 12] >> (8 - note / 12 + 1);
}
...
int main(void) {
    ...
    q31_t lfo_inc = SYNTH_HZ_TO_PHASE(4.8), vib_inc = SYNTH_HZ_TO_PHASE(0.15);
    synth_init_envelope_node(&synth_voices[0].nodes[1], NULL,
        (q31_t)(0.04 * Q31_MAX), (q31_t)(0.05 * Q31_MAX),
        (q31_t)(0.9 * Q31_MAX), (q31_t)(0.1 * Q31_MAX));
    synth_init_osc_node(&synth_voices[0].nodes[2], &vib_inc, &lfo_inc, NULL, sine_wave);
    synth_init_osc_node(&synth_voices[0].nodes[3], &synth_voices[0].nodes[1].output,
        &synth_voices[0].phase_incr, &synth_voices[0].nodes[2].output, sawtooth_wave);
    synth_init_filter_lp_node(&synth_voices[0].nodes[0],
        &synth_voices[0].nodes[3].output, svf_cutoff(900), (q31_t)(0.05 * Q31_MAX));

    // Voice 1
    synth_init_envelope_node(&synth_voices[1].nodes[1], NULL,
        (q31_t)(0.0100 * Q31_MAX), (q31_t)(0.0025 * Q31_MAX),
        (q31_t)(0.6 * Q31_MAX), (q31_t)(0.0015 * Q31_MAX));
    synth_init_osc_node(&synth_voices[1].nodes[2],
        &synth_voices[1].nodes[1].output, &synth_voices[1].phase_incr, NULL, square_wave);
    synth_init_filter_lp_node(&synth_voices[1].nodes[0],
        &synth_voices[1].nodes[2].output, svf_cutoff(1000), (q31_t)(0.95 * Q31_MAX));
